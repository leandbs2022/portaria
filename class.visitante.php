...
class visitante //classe - Funcões
 {
    function add_visita( $rg, $uf, $visitante, $tipo, $matricula, $dtcad, $destino, $placa, $modelo, $empresa, $descricao, $cor, $portaria, $datavisita, $vigilante_log )
 {
        ...
        if ( empty( $rg ) || empty( $visitante ) || empty( $descricao ) || empty( $destino ) || empty( $empresa ) || empty( $portaria ) ) {
            ...
            if ( mysqli_num_rows( $query ) ) {
                ...
                //inserir visitas
                $query = mysqli_query( $conn, "INSERT INTO `visitas`(`rg`, `destino`, `placa`, `modelo`, `empresa`, `portaria`, `data`, `resp`,`descricao`, `cor`) VALUES 
                ('$rg','$destino', '$placa','$modelo','$empresa','$portaria','$datavisita','$vigilante_log','$descricao', '$cor')" ) or die( mysqli_error( $conn ) );
