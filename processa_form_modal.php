...
if ( $_SERVER[ 'REQUEST_METHOD' ] == 'POST' ) {
    ...
    if ( empty( $rg_r ) || empty( $solicitante ) || empty( $motivo ) ) {
        ...
        if ( mysqli_num_rows( $query ) ) {
            ...
            $query = mysqli_query( $conn, "UPDATE `restrigir` SET `solicitante`='$solicitante',`telefone`=' $telefone',`rg`='$rg_r',`motivo`='$motivo',`data`='$data_r',`autorizado`='$restrigir' WHERE id='$id'" );
            ...
            if ( mysqli_num_rows( $query ) ) {
                ...
                $query = mysqli_query( $conn, "UPDATE `portaria` SET `restrigir`='$restrigir' WHERE id='$id'" );
        ...
        if ( $restrigir === 1 ) {
            ...
            if ( mysqli_num_rows( $query ) ) {
                ...
                echo "<script>alert('Esse RG não existe na tabela portaria, favor cadastra-lo na tela visitante para proceder o bloqueio!')</script>";
